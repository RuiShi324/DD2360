...
int main(int argc, char **argv) {
  ...
  cputimer_start();
  // Wikipedia explanation of CSR format: https://en.wikipedia.org/wiki/Sparse_matrix#Compressed_sparse_row_(CSR,_CRS_or_Yale_format)
  ...
  if (concurrentAccessQ) {
    ...
    // Explanation: https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__MEMORY.html#group__CUDART__MEMORY_1ge8dc9199943d421bc8bc7f473df12e42
  ...
  if (concurrentAccessQ) {
    ...
    // Explanation: https://developer.nvidia.com/blog/unified-memory-cuda-beginners/
  ...
  // Storing the time required for computing the SMPV (sparse matrix multiplication)
  float time = 0.0;
  ...
  // double FLOP = 1000*nsteps * (3*dimX - 6) / (time);
  // printf("\nFLOPS at dimx %d is %lf \n", dimX, FLOP);
